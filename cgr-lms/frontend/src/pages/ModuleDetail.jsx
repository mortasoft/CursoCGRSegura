            ...
            <div className="relative rounded-[2.5rem] overflow-hidden bg-slate-800/40 border border-white/5 shadow-2xl">
                ...
                <div className="relative z-10 p-10 md:p-16 flex flex-col md:flex-row gap-12 items-center">
                        ...
                        <div className="flex flex-wrap gap-8 pt-4">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-white font-bold">{module.lessons.length * 10} Puntos</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="w-full md:w-1/3 flex flex-col items-center">
                        ...
                        >
                            {module.completionPercentage > 0 ? 'Continuar Aprendizaje' : 'Empezar Módulo'}
                        </button>
                    </div>
                </div>
            </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="space-y-4">
                        {module.lessons.map((lesson, index) => (
                            <div
                                key={lesson.id}
                                className={`group relative p-6 rounded-2xl bg-slate-800/20 border transition-all duration-300 ${lesson.status === 'completed'
                                        ? 'border-green-500/20 bg-green-500/5'
                                        : 'border-white/5 hover:border-primary-500/40 hover:bg-slate-800/40 cursor-pointer'
                                    }`}
                                onClick={() => navigate(`/lessons/${lesson.id}`)}
                            >
                                <div className="flex items-center gap-6">
                                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors ${lesson.status === 'completed'
                                            ? 'bg-green-500/20 text-green-500'
                                            : 'bg-slate-900 text-gray-500 group-hover:text-primary-400'
                                        }`}>
                                        {lesson.lesson_type === 'video' ? <PlayCircle className="w-6 h-6" /> : <FileText className="w-6 h-6" />}
                                    </div>

                                    <div className="flex-1">
                                        <div className="flex items-center gap-3 mb-1">
                                            <span className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Lección {index + 1}</span>
                        ...
                        {module.quizzes && module.quizzes.map((quiz) => (
                            <div
                                key={quiz.id}
                                className="group relative p-6 rounded-2xl bg-secondary-500/5 border border-secondary-500/20 hover:border-secondary-500/40 hover:bg-secondary-500/10 transition-all duration-300 cursor-pointer"
                                onClick={() => navigate(`/quizzes/${quiz.id}`)}
                            >
                                <div className="flex items-center gap-6">
                                    <div className="w-12 h-12 rounded-xl bg-secondary-900/30 flex items-center justify-center flex-shrink-0 text-secondary-500">
                                        <HelpCircle className="w-6 h-6" />
                                    </div>
                                    <div className="flex-1">
                                        <div className="flex items-center gap-3 mb-1">
                                            <span className="text-[10px] font-black text-secondary-500 uppercase tracking-widest">Evaluación Final</span>
